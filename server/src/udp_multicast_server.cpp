...
// 构造函数
UdpMulticastServer::UdpMulticastServer(const std::string& multicast_address, unsigned short port, const std::string& listen_address)
	:multicast_endpoint_(net::ip::make_address(multicast_address), port),
	recv_buffer_(8192), // 增大接收缓冲区大小以处理更大的消息
...
void UdpMulticastServer::doReceive() {
	...
	socket_.async_receive_from(
		net::buffer(recv_buffer_), sender_endpoint_, // 使用sender_endpoint_接收发送者地址
		[this](boost::system::error_code ec, std::size_t bytes_transferred) {
			if (!ec && bytes_transferred > 0) {
				// 处理接收到的消息
				std::string message(recv_buffer_.data(), bytes_transferred);
				handleMessage(message, sender_endpoint_); // 传递正确的发送者地址

				// 记录接收到的消息（可选，用于调试）
				std::cout << "Received " << bytes_transferred << " bytes from "
					<< sender_endpoint_.address().to_string() << ":" << sender_endpoint_.port() << std::endl;
			}
			else if (ec) {
				if (ec != boost::asio::error::operation_aborted) {
					...
					// 尝试恢复接收操作
					if (running_) {
						std::cout << "Attempting to recover UDP receive operation..." << std::endl;
					}
				}
			}
